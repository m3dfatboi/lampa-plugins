/* subs-stable (OSv3-style, interactive tabs, ESC close, persistent settings)
   - Поиск субтитров через публичный OSv3-аддон Stremio (без ключей):
       • Сериалы: S1/S2/S3 маршруты
       • Фильмы: M1/M2 маршруты
   - Вкладки: Сериал | Фильм в одном окне
   - Настройки: размер, отступ снизу, задержка — сохраняются и применяются сразу
   - Кнопка в панели плеера:
       • Размер 2em x 2em, padding: 0.4em
       • Круглая, без обводки, расстояние от соседних .5em
       • Иконка — Myna UI Icons (MIT, Praveen Juge)
       • Полная доступность для пульта/клавиатуры (tabindex, role, Enter/Space)
*/
(function () {
  const FLAG='__SUBS_STABLE_OSV3_PLUGIN_FINAL__';
  ...
  (function(){
    ...
    // Стили в духе .player-panel .button (из скрина)
    ...
    function triggerOpen(){ try{ openModalWithTabs && openModalWithTabs('series'); }catch{} }
