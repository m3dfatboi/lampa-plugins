/* subs-stable (series manual S/E first, strict Stremio-style, UI in player controls)
   - Сериалы: сперва выбор Сезон/Серия, затем запрос сабов строго как Stremio
   - Фильмы: авто, как прежде
   - RU-сабы, "Без субтитров", live-настройки, ESC
   - Кнопка в панели плеера рядом с дефолтной субтитровой
*/
(function () {
  const FLAG='__SUBS_STABLE_SERIES_SE_STRICT__';
  ...
  function getContext(){
    ...
    // Тип определяем по наличию season/episode в Lampa — НО меню S/E всё равно будет ручным
    const type = (Number.isFinite(+ctx.season)&&+ctx.season>0 && Number.isFinite(+ctx.episode)&&+ctx.episode>0) ? 'series' : 'movie';
