/* subs-stable (OSv3-style, interactive tabs, ESC close, persistent settings)
   - Отдельная кнопка в контейнере плеера среди остальных:
     • width/height: 2em, padding: 0.4em, круглую без обводки
     • доступна для пульта: tabindex=0, role=button, Enter/Space
     • иконка — Myna UI (MIT)
   - Вкладки: Сериал | Фильм
   - Поиск через публичный OSv3-аддон (S1/S2/S3 для сериалов, M1/M2 для фильмов)
   - Настройки: размер, отступ снизу, задержка — сохраняются и применяются сразу
*/
(function () {
  const FLAG='__SUBS_STABLE_OSV3_BTN_RESTORE__';
  ...
  (function(){
    ...
    function host(){
      // контейнер правого блока контролов
      return (
    ...
    function ensure(){
      ...
      // Вставляем рядом со стандартной кнопкой субтитров, если есть; иначе — в конец контейнера
