/* subs-stable (auto, OSv3)
   - Не уменьшает видео: смещение субтитров делаем через ::cue line/position, без transform на контейнере
   - Настройки применяются сразу (live)
   - Ползунки для размера и отступа (range), задержка с шагом 0.5
*/
(function () {
  const FLAG='__SUBS_STABLE_AUTO_v3__';
  ...
  // ---- IMDb via rating.js bridge + context
  ...
  // ---- Styles: only font size via ::cue; bottom shift via cue line% (live)
  ...
  function applyCueBottom(video){
    ...
    for(const tt of video.textTracks){
      ...
      // line is percentage from top by spec; we want bottom in px.
      // Approximate: convert px->% using video height.
      const vh = video.getBoundingClientRect().height || 720;
      const px = Math.max(0, cfg.bottom);
      const percentFromBottom = (px / vh) * 100;
      const linePercent = Math.max(0, 100 - percentFromBottom); // place line this % from top
      const cues = tt.cues||[];
      for (let i=0;i<cues.length;i++){
        const c=cues[i];
        try{
          c.line = Math.min(100, Math.max(0, linePercent));
          c.align = 'center';
          c.position = 50;
          c.size = 100;
        }catch{}
  ...
  function reapplyAll(){
    ...
    applyCueBottom(v);
    // font size handled by CSS (ensureStyle)
    // delay: only re-shift when non-zero by rebuilding track (simpler: reload)
    if (lastTrackUrl){
      if (Math.abs(cfg.delay) < 1e-6) { /* no shift needed */ }
      else { const keep=lastTrackUrl; lastTrackUrl=''; applyToVideo(keep); }
    }
  }
  ...
  function openPanel(subs){
    ...
    renderList(body.querySelector('#subsList'),
      ...
      async (s)=>{
        try{
          ...
          const vtt=await toVttUrl(s.url); applyToVideo(vtt);
    ...
    sizeEl.addEventListener('input', ()=>{
      cfg.fontSize=+sizeEl.value; valSize.textContent=cfg.fontSize+'px';
      saveCfg(); ensureStyle(); reapplyAll();
    });
    bottomEl.addEventListener('input', ()=>{
      cfg.bottom=+bottomEl.value; valBottom.textContent=cfg.bottom+'px';
      saveCfg(); reapplyAll();
    });
    delayEl.addEventListener('input', ()=>{
      cfg.delay=parseFloat(delayEl.value)||0; valDelay.textContent=cfg.delay.toFixed(1);
      saveCfg(); reapplyAll();
    });
  }
