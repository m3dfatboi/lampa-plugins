/* subs-stable (OSv3-style, interactive tabs, ESC close, persistent settings)
   Запуск из дефолтного меню субтитров плеера:
   - В стандартном списке субтитров появляется пункт "Поиск субтитров (OSv3)"
   - Пункт выглядит и ведёт себя как обычный элемент меню (hover, focus, Enter/Space, пульт)
*/
(function () {
  const FLAG='__SUBS_STABLE_OSV3_MENU_INJECT_FIX__';
  ...
  async function applyToVideo(vttUrl){
    ...
    const hookTracks=()=>{
      try{
        ...
        for(const tt of v.textTracks){ tt.oncuechange=()=>{ reapply(); }; }
      }catch{}
    };
  ...
  async function querySeriesOSV3(ttRaw,sRaw,eRaw){
    ...
    try{ const r=await httpGet(OSV3,`subtitles/series/${encodeURIComponent(tt)}.json`,{season:s,episode:e}); const ru=pickRu(r.json); setDiag(`S1 ${r.href} (${ru.length})`); if(ru.length) return ru; }catch(e){}
    try{ const r=await httpGet(OSV3,'subtitles.json',{type:'series',id:tt,season:s,episode:e}); const ru=pickRu(r.json); setDiag(`S2 ${r.href} (${ru.length})`); if(ru.length) return ru; }catch(e){}
    try{ const key=`${tt}:${s}:${e}`; const r=await httpGet(OSV3,`subtitles/series/${encodeURIComponent(key)}.json`,{}); const ru=pickRu(r.json); setDiag(`S3 ${r.href} (${ru.length})`); if(ru.length) return ru; }catch(e){}
    return [];
  }
  async function queryMovieOSV3(ttRaw){
    const tt=(String(ttRaw||'').match(/^tt\d+$/)?.[0])||('tt'+String(ttRaw||'').replace(/^tt?/,'').replace(/\D.*$/,''));
    if(!tt){ setDiag(`INVALID tt ${ttRaw}`); return []; }
    try{ const r=await httpGet(OSV3,`subtitles/movie/${encodeURIComponent(tt)}.json`,{}); const ru=pickRu(r.json); setDiag(`M1 ${r.href} (${ru.length})`); if(ru.length) return ru; }catch(e){}
    try{ const r=await httpGet(OSV3,'subtitles.json',{type:'movie',id:tt}); const ru=pickRu(r.json); setDiag(`M2 ${r.href} (${ru.length})`); if(ru.length) return ru; }catch(e){}
    return [];
  }
    ...
    async function renderSeries(){
      ...
      panel.querySelector('#btnFind').onclick=async ()=>{
        try{
          ...
          renderList(list, items, async (it)=>{ if(it.off){ removeAllSubTracks(); return; } try{ const vtt=await toVttUrl(it.url); await applyToVideo(vtt); }catch{ list.textContent='Ошибка загрузки'; } });
  ...
  // ========== Встраивание в дефолтное меню субтитров ==========
  (function(){
    const ITEM_ID='subs-stable-menu-item';
    const ITEM_CLASS='subs-stable-menu-item';
    const ITEM_TEXT='Поиск субтитров (OSv3)';

    (function injectMenuStyle(){
      const sid='subs-stable-menu-style';
    ...
    function findSubtitlesMenuRoot(){
      // находим открытое меню рядом с кнопкой субтитров или явные контейнеры
      const explicit =
        document.querySelector('.player-subtitles__list, .player__subtitles .menu, .subtitles__menu, .player-menu__subtitles, .player-subtitles');
      if (explicit) return explicit;

      const btn = document.querySelector('button[aria-label="Subtitles"], button[title="Subtitles"]');
      if (!btn) return null;

      // берём ближайший видимый список/меню
      const scope = btn.closest('.player-panel, .video-player, body') || document.body;
      const candidates = scope.querySelectorAll('.menu, .list, .dropdown, .context-menu, .player-subtitles__list');
      for (const el of candidates){
        const cs = getComputedStyle(el);
        if (cs && cs.display !== 'none' && el.offsetParent !== null) return el;
      }
      return null;
    }
    ...
    function createItem(){
      ...
      el.textContent=ITEM_TEXT;

      const open = (e)=>{ if(e){ e.preventDefault(); e.stopPropagation(); } try{ openModalWithTabs('series'); }catch{} };
    ...
    function ensureItem(){
      const root=findSubtitlesMenuRoot(); if(!root) return false;
      if (document.getElementById(ITEM_ID)) return true;

      const item=createItem();
      // по умолчанию кладём последним
      root.appendChild(item);
      return true;
    }

    const mo=new MutationObserver(()=>{ try{ ensureItem(); }catch{} });
    mo.observe(document.documentElement,{childList:true,subtree:true});
    if(document.readyState==='loading'){
      document.addEventListener('DOMContentLoaded', ()=>{ try{ ensureItem(); }catch{} }, {once:true});
    } else { try{ ensureItem(); }catch{} }
    setTimeout(()=>{ try{ ensureItem(); }catch{} },300);
    setTimeout(()=>{ try{ ensureItem(); }catch{} },900);
  })();
