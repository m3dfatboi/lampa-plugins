/* subs-stable (auto, OSv3)
   Bottom offset fix: precise per-cue bottom padding using cue.line with snapToLines=false.
   Also keeps: ESC to close, "Без субтитров", live sliders for size/bottom/delay.
*/
(function () {
  const FLAG='__SUBS_STABLE_AUTO_v5__';
  ...
  // Bottom offset – robust version:
  // Use cue.line in PERCENT, computed from video height, and disable snapping so that line acts like absolute percent.
  function setCueBottomForTrack(tt, video){
    ...
    for (let i=0;i<cues.length;i++){
      ...
      try{
        // precise placement
        c.snapToLines = false;     // key for Chrome to treat `line` as percentage from top
        c.line = percent;          // 0..100 from top
        c.align = 'center';
        c.position = 50;
        c.size = 100;
      }catch{}
      ...
      for(let i=0;i<cues.length;i++){
        ...
        try{
          ...
          try{
            ...
            nc.line = c.line; nc.align=c.align; nc.position=c.position; nc.size=c.size; nc.vertical=c.vertical;
  ...
  function applyToVideo(vttUrl){
    ...
    if (!vttUrl) return; // "Без субтитров"
  ...
  function openPanel(subs){
    const items = [{label:'Без субтитров', url:'', name:'(off)'}]
      .concat(subs.map(s=>({label:s.name,url:s.url,name:s.name})));
    ...
    renderList(body.querySelector('#subsList'), items, async (s)=>{
      try{
        ...
        if (s.url){
          ...
          lastTrackUrl=''; removeAllSubTracks();
