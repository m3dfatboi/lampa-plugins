/* subs-stable (direct OpenSubtitles API; robust)
   - Прямой доступ к OpenSubtitles v1 (guest)
   - Фильмы: авто по imdb_id
   - Сериалы: ручной S/E
   - Кнопка в контролах, live-настройки, "Без субтитров"
*/
(function () {
  const FLAG='__SUBS_STABLE_DIRECT_OS_v1_FIX__';
  ...
  async function ensureToken(){
    try{
      const now=Date.now();
      if (token && now-tokenTs<50*60*1000) return token; // 50 мин
      const r=await fetch(`${OS_API}/auth/guest`, {
        ...
        body: '{}' // некоторые прокси требуют тело
      });
  ...
  async function OS_GET(path, params){
    ...
    const base=new URL(OS_API);
    const u=new URL(path.replace(/^\//,''), base.toString()+'/');
  ...
  async function OS_POST(path, body){
    const t=await ensureToken();
    const u=new URL(path.replace(/^\//,''), OS_API+'/').toString();
    ...
    if(!r.ok) throw new Error('POST '+u+' '+r.status);
    return r.json();
  }
  ...
  async function searchMovie(imdb){
    ...
    }catch(e){ console.warn('searchMovie', e); return []; }
  }
  ...
  async function searchSeries(imdb, season, episode){
    try{
      ...
      return pickRUfromOS(resp);
    }catch(e){ console.warn('searchSeries', e); return []; }
  }
  ...
  function closeModal(){
    const w=document.getElementById('subs-stable-modal'); if(w) w.remove(); modalOpen=false;
  }
  ...
  function bindSettings(root){
    ...
    }catch(e){ console.error('bindSettings', e); }
  }
  ...
  async function openMovie(){
    try{
      ...
      openModal(`<div>Ошибка: ${safe(e.message||e)}</div>`);
    }
  }
        ...
        try{
          ...
          listButtons(list, items, async (it)=>{
            try{
              ...
              const direct = await downloadDirectUrl(it.file_id);
              await applyUrl(direct);
            }catch{ list.textContent='Ошибка загрузки'; }
          });
        }catch{ list.textContent='Ошибка поиска'; }
      };
    }catch(e){
      openModal(`<div>Ошибка: ${safe(e.message||e)}</div>`);
    }
  ...
  async function openRoot(){
    openModal(`
      ...
      <div id="panel"></div>
    `);
    const bS=document.getElementById('btnSeries');
    const bM=document.getElementById('btnMovie');
    bS && (bS.onclick=()=>openSeries());
    bM && (bM.onclick=()=>openMovie());
  }
  ...
  // ------ Init ------
  if(document.readyState==='loading'){
    document.addEventListener('DOMContentLoaded', ()=>{ try{ ensureStyle(); }catch{} }, {once:true});
  } else { try{ ensureStyle(); }catch{} }
})();
