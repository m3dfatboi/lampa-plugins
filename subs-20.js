/* subs-stable (series manual S/E select -> Stremio-style request)
   - Фильмы: авто (как раньше)
   - Сериалы: сначала ручной выбор Сезон/Серия в меню, затем запрос сабов строго в стиле Stremio:
       GET /subtitles/series/{imdb}.json?season=S&episode=E
     + fallback: /subtitles.json?type=series&id=tt...&season=&episode=
   - Остальное без изменений: RU-сабы, "Без субтитров", live-настройки, ESC, кнопка в контролах
*/
(function () {
  const FLAG='__SUBS_STABLE_SERIES_MANUAL_SE__';
  ...
  async function fetchSeriesExact(imdb, s, e, title, year){
    const tries=[
  ...
  async function openMoviePanel(meta){
    const subs = await fetchMovie(meta.imdb, meta.seriesTitle, meta.year);
  ...
  async function openSeriesPanel(meta){
    // Ручной выбор S/E до запроса — “эмуляция Stremio”
    const defaultS = meta.season||1, defaultE = meta.episode||1;

    const {body}=openModal(`
      <div style="display:grid;gap:12px;">
        <fieldset style="border:1px solid #333;border-radius:10px;padding:10px;">
          <legend style="padding:0 6px;color:#bbb;">Сначала выберите сезон и серию</legend>
          <div id="sePick" style="display:flex;gap:10px;align-items:center;flex-wrap:wrap;margin-bottom:8px;">
            <label>Сезон:
              <select id="selSeason" style="margin-left:6px;padding:6px 10px;background:#111;color:#fff;border:1px solid #444;border-radius:8px;">
                ${Array.from({length:30},(_,i)=>i+1).map(v=>`<option value="${v}" ${v===defaultS?'selected':''}>${v}</option>`).join('')}
              </select>
            </label>
            <label>Серия:
              <select id="selEpisode" style="margin-left:6px;padding:6px 10px;background:#111;color:#fff;border:1px solid #444;border-radius:8px;">
                ${Array.from({length:60},(_,i)=>i+1).map(v=>`<option value="${v}" ${v===defaultE?'selected':''}>${v}</option>`).join('')}
              </select>
            </label>
    ...
    async function loadForSelected(){
      ...
      if (!subs.length){
        subs = await fetchSeriesExact(meta.imdb, s, e, meta.seriesTitle, meta.year);
      }
  ...
  // ---------- Button inside player controls (как ранее) ----------
  (function(){
