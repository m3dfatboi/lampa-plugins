/* subs-stable (series manual S/E + strict Stremio style; movies auto; UI in player controls)
   - Кнопка рядом с дефолтной CC в панели плеера
   - Фильмы: авто-поиск RU субтитров
   - Сериалы: сначала ручной выбор Сезон/Серия, затем строгий запрос:
       GET /subtitles/series/{imdb}.json?season=S&episode=E
       fallback: /subtitles.json?type=series&id=tt...&season=&episode=
   - Пункт "Без субтитров"
   - Live-настройки (размер, отступ снизу, задержка); отступ через VTTCue.snapToLines=false
   - ESC закрывает окно
*/
(function () {
  const FLAG='__SUBS_STABLE_FINAL_SINGLE_v2__';
  ...
  async function fetchMovie(imdb, title, year){
    ...
    for (const t of tries){
      try{ const d=await GET(t.p,t.q); const ru=pickRu(d); if (ru.length){ subs=ru; break; } }catch{}
    }
    return subs;
  }
      ...
      <fieldset style="border:1px solid #333;border-radius:10px;padding:10px;">
        ...
        <div style="display:grid;gap:14px;grid-template-columns:1fr 1fr;">
          ...
          <div style="grid-column:1/-1;">
            <label>Задержка (сек): <span id="valDelay">${cfg.delay.toFixed(1)}</span></label>
  ...
  function bindSliders(root){
    const sizeEl=root.querySelector('#rangeSize');
    const bottomEl=root.querySelector('#rangeBottom');
    const delayEl=root.querySelector('#rangeDelay');
    const valSize=root.querySelector('#valSize');
    const valBottom=root.querySelector('#valBottom');
    const valDelay=root.querySelector('#valDelay');
  ...
  // ---------- Series controls (always visible) ----------
  function renderSeriesControls(meta, mountNode, onSearch){
    mountNode.innerHTML = `
      <fieldset style="border:1px solid #333;border-radius:10px;padding:10px;">
        ...
        <div id="subsStableImdbWarn" style="color:#f78;display:${meta.imdb ? 'none' : ''};">Внимание: не найден IMDb ID — результаты могут быть пустыми. Откройте экран с рейтингами, чтобы подтянулся imdb_id.</div>
      </fieldset>
    `;
  ...
  async function openMoviePanel(meta){
    ...
    // Рендерим ВСЕГДА S/E блок сверху (на случай, если это всё-таки сериал)
  ...
  async function loadSeriesFor(meta, season, episode, listNode){
    ...
    if (meta.imdb){
      try{
        const d=await GET(`subtitles/series/${encodeURIComponent(meta.imdb)}.json`, {season, episode});
        subs = pickRu(d);
      }catch{}
    }
    if (!subs.length){
      subs = await fetchSeriesStrict(meta.imdb, season, episode, meta.title, meta.year);
    }
  ...
  async function openAuto(){
    try{
      ...
      // Не пытаемся угадывать тип — рисуем сериал-панель, если пользователь хочет; параллельно показываем фильм-поиск в movie-панели.
      // По умолчанию откроем сериал-панель (как просили — ручное управление), если нужно — есть автопоиск в movie-панели:
