/* subs-stable (OSv3-style, interactive tabs, ESC close, persistent settings + robust bottom apply)
   - Вкладки: Сериал | Фильм (двустороннее переключение в одном окне)
   - Сериалы: S/E -> 3 строгих OSv3 маршрута:
       1) /subtitles/series/{tt}.json?season=&episode=
       2) /subtitles.json?type=series&id=tt&season=&episode=
       3) /subtitles/series/{tt:season:episode}.json
   - Фильмы: /subtitles/movie/{tt}.json (+fallback /subtitles.json?type=movie&id=tt)
   - Настройки (live) и их реальное применение:
   ...
   - “Без субтитров”, закрытие по Esc, кнопка в контролах плеера
*/
(function () {
  const FLAG='__SUBS_STABLE_OSV3_FULL_ROBUST__';
  ...
  async function applyToVideo(vttUrl){
    ...
    // observe DOM for track changes (players may recreate track/cues)
  ...
  function bindSettings(root){
    ...
    rs?.addEventListener('input', ()=>{
      ...
      ensureStyle();
      // пересчёт отступа не нужен для размера
    });
