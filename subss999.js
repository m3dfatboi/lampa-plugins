(function () {
  ...
  const OSV3='https://opensubtitles.stremio.com/stremio/v1/';
  const LANG='ru';
  ...
  async function querySeriesOSV3(ttRaw,sRaw,eRaw){
    ...
    try{ const r=await httpGet(OSV3,`q`,{imdbId:tt,season:s,episode:e}); const ru=pickRu(r.json); setDiag(`S1 ${r.href} (${ru.length})`); if(ru.length) return ru; }catch(e){setDiag(`S1 Error: ${e}`);}
    try{ const key=`${tt}:${s}:${e}`; const r=await httpGet(OSV3,`q`,{imdbId:key}); const ru=pickRu(r.json); setDiag(`S3 ${r.href} (${ru.length})`); if(ru.length) return ru; }catch(e){setDiag(`S3 Error: ${e}`);}
    return [];
  }
  async function queryMovieOSV3(ttRaw){
    const tt=(String(ttRaw||'').match(/^tt\d+$/)?.)||('tt'+String(ttRaw||'').replace(/^tt?/,'').replace(/\D.*$/,''));
    if(!tt){ setDiag(`INVALID tt ${ttRaw}`); return []; }
    try{ const r=await httpGet(OSV3,`q`,{imdbId:tt}); const ru=pickRu(r.json); setDiag(`M1 ${r.href} (${ru.length})`); if(ru.length) return ru; }catch(e){setDiag(`M1 Error: ${e}`);}
    return [];
  }
