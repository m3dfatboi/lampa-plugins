(function () {
  ...
  // ====== SubHero base (закреплён язык ru из твоей ссылки) ======
  const STREMIO_LINK = 'stremio://subhero.onrender.com/%7B%22language%22%3A%22ru%22%7D/manifest.json';
  function normalizeSubHeroBase(input) {
    let s = String(input || '').trim();
    if (!s) return 'https://subhero.onrender.com/';
    s = s.replace(/^stremio:\/\//i, 'https://');
    s = s.replace(/manifest\.json\s*$/i, '');
    s = s.replace(/[?#].*$/, '');
    if (!s.endsWith('/')) s += '/';
    return s;
  }
  const SUBHERO_BASE = normalizeSubHeroBase(STREMIO_LINK);
  const LANG='ru';
  ...
  // ====== SubHero Queries (Stremio subtitles resource) ======
  function pickRuFromStremio(d){
  ...
  async function queryMovieSubHero(ttRaw){
  ...
  function openModalWithTabs(defaultTab='series'){
    ...
    async function renderSeries(){
      ...
      panel.querySelector('#btnFind').onclick=async ()=>{
        try{
          ...
          const ru=await querySeriesSubHero(imdbNow,s,e);
    ...
    async function renderMovie(){
      ...
      try{
        ...
        const ru=await queryMovieSubHero(imdbNow);
        if(!ru.length){ list.textContent='Субтитры не найдены'; return; }
        const items=[{label:'Без субтитров', off:true}].concat(ru.map(x=>({label:x.name,url:x.url})));
        renderList(list, items, async (it)=>{
          if(it.off){ removeAllSubTracks(); return; }
