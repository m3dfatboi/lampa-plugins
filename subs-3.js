/* subs-stable (auto-detect)
   - Автоопределение текущего тайтла из плеера/активности Lampa (IMDb/тип/S-E)
   - Список RU субтитров сразу после нажатия CC
   - Панель настроек под списком (размер, отступ снизу, задержка ±0.5s)
   - Исправлен отступ снизу (применяется к контейнеру трека)
*/
(function () {
  const FLAG='__SUBS_STABLE_AUTO__';
  ...
  // -------- detect current title from Lampa context
  function getContextMeta(){
    const act = (window.Lampa?.Player?.video && Lampa.Player.video())
             || (window.Lampa?.Activity?.active && Lampa.Activity.active())
             || {};
    const imdbRaw = safe(act.imdb_id || act.imdb);
    const imdb = imdbRaw ? (imdbRaw.startsWith('tt')?imdbRaw:('tt'+imdbRaw.replace(/^tt/,''))) : '';
    const meta = {
      title: safe(act.title || act.name),
      year: act.year ? +act.year : (act.release_year ? +act.release_year : ''),
      season: Number.isFinite(+act.season)?+act.season:0,
      episode: Number.isFinite(+act.episode)?+act.episode:0,
      imdb_id: imdb
    };
  ...
  async function fetchAutoSubs(){
    ...
    // fallback: try by title EN/RU (with year)
    const searches = [];
    if (meta.title) {
      if (meta.year) searches.push(`${meta.title} ${meta.year}`);
      searches.push(meta.title);
    }
    for (const s of searches){
      tries.push({p:'subtitles.json', q:{type, query:s, season:meta.season||'', episode:meta.episode||''}});
    }
  ...
  // -------- styles (font size + fixed bottom offset)
  function ensureStyle(){
    ...
    // Используем контейнер-подкладку, чтобы гарантированно задать нижний отступ
    st.textContent=`
      #subs-stable-wrapper{ position:fixed; left:0; right:0; bottom:${cfg.bottom}px; height:0; pointer-events:none; z-index:2147483646; }
      video::-webkit-media-text-track-display { font-size:${cfg.fontSize}px !important; }
      video::cue { font-size:${cfg.fontSize}px; }
    `;
    // wrapper создаётся один раз
    if (!document.getElementById('subs-stable-wrapper')){
      const w=document.createElement('div'); w.id='subs-stable-wrapper'; document.body.appendChild(w);
    }
  }
  ...
  // -------- modal with subs list + settings below
    ...
    renderList(body.querySelector('#subsList'),
      ...
      async (s)=>{
        try{
          ...
          try{ window.Lampa?.Noty?.show && Lampa.Noty.show('Ошибка загрузки субтитров'); }catch{}
        }
      }
    );
