/* subs-stable (OSv3-style + top slider Movie/Series, ESC close)
   - Верхний слайдер-переключатель: Сериал | Фильм (в одном экране)
   - Сериалы: ручной выбор S/E → три строгих маршрута OSv3 (+ диагностика по флагу)
   - Фильмы: OSv3 movie + fallback subtitles.json
   - Настройки (live), "Без субтитров"
   - Закрытие окна по Esc
*/
(function () {
  const FLAG='__SUBS_STABLE_OSV3_SLIDER_ESC__';
  ...
  // ---------- Diagnostics ----------
  function diagBlock(){ return `<div id="subsDiag" style="font-size:12px;color:#aaa;margin:6px 0;"></div>`; }
  ...
  async function querySeriesOSV3(tt, s, e){
    try{
      ...
      if (cfg.debug) setDiag(`S1 ${r.href} (${ru.length})`);
    ...
    try{
      ...
      const ru=pickRu(r.json);
      if (cfg.debug) setDiag(`S2 ${r.href} (${ru.length})`);
      if(ru.length) return ru;
    ...
    try{
      ...
      const ru=pickRu(r.json);
      if (cfg.debug) setDiag(`S3 ${r.href} (${ru.length})`);
      if(ru.length) return ru;
  ...
  // ---------- UI: unified modal with slider ----------
  function sliderTabsHTML(active){
    const aS = active==='series';
    return `
      <div id="tabSwitch" style="display:flex;gap:6px;margin-bottom:10px;">
        <button id="tabSeries" style="flex:1;padding:10px;border-radius:10px;border:1px solid #444;cursor:pointer;${aS?'background:#0a84ff;border-color:#0a84ff;color:#fff;':'background:#222;color:#fff;'}">Сериал</button>
        <button id="tabMovie"  style="flex:1;padding:10px;border-radius:10px;border:1px solid #444;cursor:pointer;${!aS?'background:#0a84ff;border-color:#0a84ff;color:#fff;':'background:#222;color:#fff;'}">Фильм</button>
      </div>
    `;
  }
  ...
  function openUnifiedModal(defaultTab){
    ...
    document.body.appendChild(w); w.appendChild(b);

    // ESC закрытие
    const onKey=(e)=>{ if(e.key==='Escape'){ e.preventDefault(); e.stopPropagation(); close(); } };
    window.addEventListener('keydown', onKey, true);
    w.addEventListener('click', e=>{ if(e.target===w) close(); });
    function close(){ window.removeEventListener('keydown', onKey, true); w.remove(); }

    // контент
    b.innerHTML = `
      <h3 style="margin:0 0 8px 0;">${LABEL}</h3>
      ${sliderTabsHTML(defaultTab)}
      ${cfg.debug?diagBlock():''}
      <div id="panel"></div>
      ${settingsBlock()}
    `;
    ...
    // вкладки
    const tabSeriesBtn=b.querySelector('#tabSeries');
    const tabMovieBtn =b.querySelector('#tabMovie');
    const panel=b.querySelector('#panel');

    async function renderSeries(){
      b.querySelector('#tabSwitch').outerHTML = sliderTabsHTML('series');
      const sHtml = `
      ...
      panel.querySelector('#btnFind').onclick=async ()=>{
        const s=+panel.querySelector('#season').value;
        const e=+panel.querySelector('#episode').value;
      ...
      // перевесить обработчики табов
      b.querySelector('#tabSeries').onclick=renderSeries;
      b.querySelector('#tabMovie').onclick =renderMovie;
    }

    async function renderMovie(){
      b.querySelector('#tabSwitch').outerHTML = sliderTabsHTML('movie');
      const mHtml = `
        <fieldset style="border:1px solid #333;border-radius:10px;padding:10px;">
          <legend style="padding:0 6px;color:#bbb;">Субтитры</legend>
      ...
      `;
      panel.innerHTML=mHtml;
      const list=panel.querySelector('#subsList');

      const tt=normImdb(imdb);
      if(!tt){ list.textContent='Нет imdb_id'; return; }
      try{
        const ru=await queryMovieOSV3(tt);
        if(!ru.length){ list.textContent='Субтитры не найдены'; return; }
        const items=[{label:'Без субтитров', off:true}].concat(ru.map(x=>({label:x.name, url:x.url})));
        listButtons(list, items, async (it)=>{ if(it.off){ removeTracks(); return; } try{ await applyUrl(it.url); }catch{ list.textContent='Ошибка загрузки'; } });
      }catch{ list.textContent='Ошибка запроса'; }
  ...
  (function(){
    ...
    function ensure(){
      ...
      b.onclick=(e)=>{ e.preventDefault(); e.stopPropagation(); openUnifiedModal("series"); };
      h.appendChild(b);
      return true;
    }
