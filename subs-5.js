/* subs-stable (auto-detect, OSv3)
   + Берёт imdb_id из плагина рейтингов @t_anton (rating.js), если доступен
   + Автосписок RU субтитров + панель настроек (размер, отступ, задержка)
*/
(function () {
  const FLAG='__SUBS_STABLE_IMDB_BRIDGE__';
  ...
  // -------- IMDb bridge: пытаемся забрать imdb_id из rating.js
  function getImdbFromRatingPlugin(){
    try{
      // 1) Через активный экран Lampa
      ...
      // 2) Через render DOM, если rating.js уже отработал (иногда он сохраняет в data-атрибутах)
      const render = act?.activity?.render && act.activity.render();
      if (render){
        const n1 = render.querySelector('[data-imdb], [data-imdb-id]');
        const v1 = n1?.getAttribute('data-imdb') || n1?.getAttribute('data-imdb-id');
        if (v1) return normImdb(v1);
      }
    }catch{}
    return '';
  }
  function normImdb(x){
    const s = safe(x);
    if (!s) return '';
    return s.startsWith('tt') ? s : ('tt'+s.replace(/^tt/, ''));
  }
  ...
  function getContextMeta(){
    // приоритет: rating.js -> Player.video() -> Activity.active() -> document.title
    const imdbFromRating = getImdbFromRatingPlugin();
    ...
    let imdbRaw = imdbFromRating || safe(ctx.imdb_id || ctx.imdb);
    imdbRaw = imdbRaw ? normImdb(imdbRaw) : '';
