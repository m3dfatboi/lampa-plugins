/* subs-stable (direct OpenSubtitles API; stable UI in player controls)
   - Источник: api.opensubtitles.com (v1, guest auth)
   - Фильмы: поиск по imdb_id (RU), выбор и применение
   - Сериалы: ручной выбор Сезон/Серия → поиск по imdb_id+season+episode (RU)
   - Кнопка рядом с дефолтной кнопкой субтитров
   - Настройки: размер, отступ снизу, задержка; "Без субтитров"; ESC
   Примечания:
   - ZIP-архивы пропускаются (для простоты). Большинство релизов — .srt/.vtt.
   - Для IMDb обязателен формат tt1234567 (вытаскиваем из Lampa/rating.js).
*/
(function () {
  const FLAG='__SUBS_STABLE_DIRECT_OS_v1__';
  ...
  let token='', tokenTs=0, modalOpen=false, escBound=false, videoObs=null;

  // ---------------- OpenSubtitles auth (guest) ----------------
  async function ensureToken(){
    const now=Date.now();
    if (token && now-tokenTs<60*60*1000) return token;
    ...
    if (!r.ok) throw new Error('opensubtitles auth '+r.status);
  ...
  async function OS_GET(path, params){
    ...
    if(params) Object.keys(params).forEach(k=>{
      const v=params[k]; if(v!==''&&v!=null) u.searchParams.set(k, String(v));
    });
    const r=await fetch(u.toString(), {
      headers:{ 'Accept':'application/json', 'Authorization':`Bearer ${t}`, 'User-Agent':APP_AGENT }
    });
    if(!r.ok) throw new Error('opensubtitles get '+r.status);
    return r.json();
  }
  ...
  async function OS_POST(path, body){
    ...
    if(!r.ok) throw new Error('opensubtitles post '+r.status);
    return r.json();
  }
  ...
  function pickRUfromOS(resp){
    ...
    const ru = data.filter(x=>{
      const lang=(x.attributes?.language||'').toLowerCase();
      return lang==='ru' && x.attributes?.files?.[0]?.file_id;
    }).map(x=>{
      const file = x.attributes.files?.;
      return {
        id: x.id,
        name: x.attributes?.release || x.attributes?.feature_details || x.attributes?.title || ('OS #'+x.id),
        file_id: file?.file_id,
        file_name: file?.file_name||'',
        ext: (file?.file_name||'').split('.').pop().toLowerCase()
      };
    });
    return ru;
  }
  ...
  async function searchMovie(imdb){
    const id = normImdb(imdb).replace(/^tt/,'');
    if(!id) return [];
    const resp = await OS_GET('/subtitles', {
      imdb_id:id, languages:'ru', order_by:'downloads', order_direction:'desc'
    });
  ...
  async function searchSeries(imdb, season, episode){
    ...
    const resp = await OS_GET('/subtitles', {
      imdb_id:id, season, episode, languages:'ru', order_by:'downloads', order_direction:'desc'
    });
    return pickRUfromOS(resp);
  }
  ...
  async function toVttObjectUrl(url){
    ...
    let t='';
    try{ t=new TextDecoder('utf-8').decode(b); }
    catch{ t=new TextDecoder('windows-1251').decode(new Uint8Array(b)); }
  ...
  function openModal(html){
    ...
    if(!escBound){
      escBound=true;
      window.addEventListener('keydown',(e)=>{ if(e.key==='Escape'&&modalOpen){ e.preventDefault(); e.stopPropagation(); closeModal(); }}, true);
    }
  }
    ...
    return `
      <fieldset style="border:1px solid #333;border-radius:10px;padding:10px;margin-top:10px;">
        ...
        <div style="display:grid;gap:14px;grid-template-columns:1fr 1fr;">
          <div>
            <label>Размер: <span id="valSize">${cfg.fontSize}px</span></label>
            <input id="rSize" type="range" min="10" max="60" step="1" value="${cfg.fontSize}" style="width:100%;">
          </div>
          <div>
            <label>Отступ снизу: <span id="valBottom">${cfg.bottom}px</span></label>
            <input id="rBottom" type="range" min="0" max="300" step="2" value="${cfg.bottom}" style="width:100%;">
          </div>
          <div style="grid-column:1/-1;">
            <label>Задержка (сек): <span id="valDelay">${(cfg.delay||0).toFixed(1)}</span></label>
            <input id="rDelay" type="range" min="-10" max="10" step="0.5" value="${cfg.delay||0}" style="width:100%;">
          </div>
        </div>
      </fieldset>
    `;
  ...
  async function openMovie(){
    const meta=getMeta();
    const ru = await searchMovie(meta.imdb);
    openModal(`
      ...
      <fieldset style="border:1px solid #333;border-radius:10px;padding:10px;">
        ...
        <div id="list">${ru.length?'':'Субтитры не найдены'}</div>
      </fieldset>
      ${settingsBlock()}
    `);
    ...
    const items=[{label:'Без субтитров', off:true}].concat(
      ru.map(x=>({label:x.name, file_id:x.file_id}))
    );
    listButtons(document.getElementById('list'), items, async (it)=>{
      try{
        if (it.off){ removeTracks(); return; }
        const direct = await downloadDirectUrl(it.file_id);
        await applyUrl(direct);
      }catch{ /* ignore */ }
    });
  }
  ...
  async function openSeries(){
    ...
    openModal(`
      ...
      <fieldset style="border:1px solid #333;border-radius:10px;padding:10px;">
        ...
        <div style="color:#f78;${meta.imdb?'display:none;':''};margin-top:8px;">IMDb ID не найден — откройте экран рейтингов, чтобы подтянуть id.</div>
      </fieldset>
    ...
    document.getElementById('find').onclick = async ()=>{
      ...
      try{
        const ru=await searchSeries(meta.imdb, s, e);
        ...
        list.textContent='Ошибка поиска';
      }
    };
  }
