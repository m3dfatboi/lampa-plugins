(function () {
  const FLAG='__SUBS_STABLE_SUBHERO_RU_BTN_FILLHEIGHT__';
  if (window[FLAG]) return; window[FLAG]=true;

  // ====== SubHero base (язык RU из твоей ссылki) ======
  ...
  const SUBHERO_BASE = normalizeBase(STREMIO_LINK);
  const LANG='ru';
  ...
  // ====== HTTP helpers with warmup + retries ======
  ...
  async function httpGet(base, path, q){
    ...
    for (let attempt=0; attempt<3; attempt++){
      try{
        const json = await fetchJSON(href);
        return { json, href };
      } catch(e){
        lastErr = e;
        // холодный старт или временные 5xx — попробуем разбудить и подождать
        if (attempt===0){ try{ await warmup(); }catch{} }
  ...
  async function warmup(){
    // будим аддон, игнорируем ошибки
    try{ await fetchJSON(new URL('manifest.json', SUBHERO_BASE).toString()); }catch{}
  }
  ...
  async function tryPaths(paths){
    ...
    for (const p of paths){
      try{
        const r = await httpGet(SUBHERO_BASE, p.path, p.query||null);
        const items = normalizeSubsResponse(r.json);
        const ru = pickRu(items);
        setDiag(`URL ok: ${r.href} (total:${items.length} ru:${ru.length})`);
        if (ru.length) return ru;
      }catch(e){
        lastErr = e;
        setDiag(`URL fail: ${p.path} ${e.message||e}`);
      }
    }
  ...
  async function querySeries(ttRaw, sRaw, eRaw){
    ...
    const id = `${tt}:${s}:${e}`;
    await warmup();
    // типовые варианты маршрутов
    const paths = [
      { path: `subtitles/series/${encodeURIComponent(id)}.json` },
      { path: `subtitles/${encodeURIComponent(id)}.json`, query:{ type:'series' } },
      { path: `subtitles/series/${encodeURIComponent(tt)}.json`, query:{ season:s, episode:e, language:LANG } },
      { path: `subtitles.json`, query:{ type:'series', id:id, language:LANG } },
      { path: `q`, query:{ type:'series', id:id, language:LANG } },
    ];
    try{ return await tryPaths(paths); }catch{ return []; }
  }
  ...
  async function queryMovie(ttRaw){
    ...
    await warmup();
    const paths = [
      { path: `subtitles/movie/${encodeURIComponent(tt)}.json` },
      { path: `subtitles/${encodeURIComponent(tt)}.json`, query:{ type:'movie' } },
      { path: `subtitles.json`, query:{ type:'movie', id:tt, language:LANG } },
      { path: `q`, query:{ type:'movie', id:tt, language:LANG } },
      { path: `subtitles/movie/${encodeURIComponent(tt)}.json`, query:{ language:LANG } },
    ];
    try{ return await tryPaths(paths); }catch{ return []; }
  }
