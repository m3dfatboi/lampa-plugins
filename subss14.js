...
*/
(function () {
  const FLAG='__SUBS_STABLE_OSV3_MENU_INJECT__';
  ...
  (function(){
    ...
    // Стили под стандартный пункт меню
    ...
    function findSubtitlesMenuRoot(){
      // типовые варианты корневого списка/меню субтитров
      return (
        document.querySelector('.player-subtitles__list') ||
        document.querySelector('.player__subtitles .menu, .subtitles__menu, .player-menu__subtitles, .player-subtitles') ||
        // fallback — любые открытые меню рядом с кнопкой субтитров
        (()=> {
          const btn = document.querySelector('button[aria-label="Subtitles"], button[title="Subtitles"]');
          if (!btn) return null;
          // ближайшее всплывшее меню
          const menu = btn.closest('.player-panel, .video-player')?.querySelector('.menu, .list, .dropdown, .context-menu');
          return menu || null;
        })()
      );
    }
    ...
    function createItem(){
      ...
      const open = (e)=>{ e?.preventDefault?.(); e?.stopPropagation?.(); try{ openModalWithTabs('series'); }catch{} };
    ...
    function ensureItem(){
      ...
      // куда вставлять: перед первой строкой или в конец — выбери вариант
