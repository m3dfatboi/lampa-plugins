/* subs-stable (all-in-one, OSv3)
   - Автоопределение (IMDb из rating.js/контекста), фильмы и сериалы (ручной выбор S/E)
   - Список RU субтитров + пункт "Без субтитров"
   - Live-настройки: размер, отступ снизу, задержка; точный bottom через snapToLines=false
   - ESC закрывает окно
   - Кнопка видима только в плеере (когда есть <video>), автоскрытие через 5s без активности
*/
(function () {
  const FLAG='__SUBS_STABLE_ALLINONE_FINAL__';
